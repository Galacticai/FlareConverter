...
import global.common.util.IOUtil.children
import java.io.File
...
object IOUtil {
    ...
    fun InputStream.copyToFile(destination: File, overwrite: Boolean = false): Long? {
        Log.d(
            IOUtil::class.java.simpleName,
            "Copying ${this::class.java.simpleName} to $destination"
        )
        use { i ->
    ...
    fun Uri.copyToFile(
        ...
        ?.copyToFile(destination, overwrite)
    ...
    fun File.copyToFile(destination: File, overwrite: Boolean = false): Long? {
        Log.d(IOUtil::class.java.name, "Copying $this to $destination")
    ...
    fun File.moveToFile(destination: File, overwrite: Boolean = false): Long? {
        Log.d(IOUtil::class.java.simpleName, "Moving $this to $destination")
    ...
    fun File.deleteAfter(milliseconds: Long) {
        Log.d(IOUtil::class.java.simpleName, "Deleting $this after $milliseconds ms")
    ...
    /** Get the mime type from this [extension] */
    val File.mime: String?
        get() = this.extension.mimeFromExtension
     ...
     * @throws java.io.IOException */
    fun File.deleteChildrenRecursively(recreate: Boolean = false) {
    ...
    fun File.toExtension(extension: String) =
