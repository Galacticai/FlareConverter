...
import global.common.models.FutureValue.Companion.stop
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch
import java.io.File

typealias InformedFile = Pair<File, MediaInformation>
...
//    val mediaInfoState = mutableStateOf<MediaInformation?>(null)
    ...
    val inFileLive = FutureValue.live<InformedFile>()

    /** The converted file in the app output directory ([outputDir]) */
    val outFileLive = FutureValue.live<InformedFile>()

    val configScrollProgressState = mutableFloatStateOf(0f)
    val configExpandStates = mutableListOf<MutableState<Boolean>>()

    val liveAll get() = listOf(inFileLive, outFileLive)
    val liveRunning get() = liveAll.filter { it.value is FutureValue.Running }
    fun stopAll() {
