...
import com.galacticai.flareconverter.ui.MimeTypeView
import com.galacticai.flareconverter.ui.MimeTypeViewStyle
...
import com.galacticai.flareconverter.util.AppDefaults.exportFile
import com.galacticai.flareconverter.util.Consistent
import com.galacticai.flareconverter.util.MimeTypeUtils
import com.galacticai.flareconverter.util.MimeTypeUtils.dashed
import com.galacticai.flareconverter.util.MimeTypeUtils.outMimeSetting
import global.common.ActivityUtil.openFile
import global.common.ActivityUtil.shareFile
...
private fun SheetContent() {
    val activity = LocalActivity.current as ConvertActivity
    val vm = activity.vm
    val helpers = activity.helpers
                ...
                Surface(
                    ...
                    AnimatedContent(
                        ...
                        when (inFileFrameFuture) {
                            ...
                            is FutureValue.Finished -> {
                                ImageBoxView(inFileFrameFuture.finishedValue!!)
                            }
                        }
                    }
                }

                MimeTypeFromToView(inMime!!, outMime)
...
@Composable
private fun ImageBoxView(inFile: File) {
    val activity = LocalActivity.current as ConvertActivity
    ...
    Box(
        modifier = Modifier
            ...
            .heightIn(max = 500.dp)
            .clickable {
                val exported = activity.exportFile(inFile)
                activity.openFile(exported)
            }
    ) {
        Image(
            rememberAsyncImagePainter(inFile.absolutePath),
            null,
...
@Composable
private fun MimeTypeFromToView(inMime: MimeType, outMime: MimeType?) {
    Row(
        Modifier
        ...
        MimeTypeView(inMime, title = "From")
        Icon(
            Icons.Rounded.PlayArrow,
            contentDescription = "conversion arrow",
        )
        MimeTypeView(
            outMime ?: MimeType.Anything,
            title = "To",
