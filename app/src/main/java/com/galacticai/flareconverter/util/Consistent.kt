...
import com.galacticai.flareconverter.ui.components.expressive.expressive_card.ExpressiveRadius
...
object Consistent {
    ...
    object Shape {
        object Radius {
            ...
            val expressive = ExpressiveRadius(100f, 30f)
    ...
    object Animation {
        fun <T> getExpressiveSpring(bounce: Boolean = false) = spring<T>(
            dampingRatio = if (bounce) Spring.DampingRatioMediumBouncy else Spring.DampingRatioNoBouncy,
            stiffness = Spring.StiffnessMedium,
        )

        val slideUpDown: ContentTransform
            get() = (fadeIn() + slideInVertically(initialOffsetY = { it })) togetherWith
